...
import { Theme, createStyles, WithStyles, withStyles, AppBar, Toolbar, Typography, Paper, colors, Button } from '@material-ui/core';
...
type ChangeTypes = 'bug' | 'feature' | 'enhancement';

const CHANGE_TYPES: { [ changeType in ChangeTypes ]: string } = {
  bug: 'Bug',
  feature: 'Feature',
...
function arrayable<T>( arr: Arrayable<T> ): T[]
{
...
const changelogEntryStyles = ( theme: Theme ) => createStyles( {
  changeType: {
...
const UnstyledChangelogEntry: React.SFC<{
  changeType?: Arrayable<ChangeTypes>;
  text: React.ReactNode;
  issueNumber?: Arrayable<number>;
  image?: Arrayable<{ src: string, alt: string }>;
} & WithStyles<typeof changelogEntryStyles>> = ( { classes, changeType, text, issueNumber, image } ) => (
  <li>
    {typeof changeType !== 'undefined' && (
      arrayable( changeType ).map( ( t ) => (
        <Typography
          key={t}
          className={classNames( classes.changeType, classes[ t ] )}
          variant="caption"
          component="span"
          inline={true}
          paragraph={false}
    ...
    {( typeof issueNumber !== 'undefined' ) && (
      arrayable( issueNumber ).map( ( num ) => (
        <Button
          ...
          variant="text"
          size="small"
          href={`https://github.com/anickle060193/easy_control/issues/${num}`}
          target="_blank"
...
const ChangelogEntry = withStyles( changelogEntryStyles )( UnstyledChangelogEntry );

const changelogVersionStyles = ( theme: Theme ) => createStyles( {
  root: {
    '&:not( :last-child )': {
...
const UnstyledChangelogVersion: React.SFC<{
  version: string;
} & WithStyles<typeof changelogVersionStyles>> = ( { classes, version, children } ) => (
...
const ChangelogVersion = withStyles( changelogVersionStyles )( UnstyledChangelogVersion );

class ChangelogPage extends React.Component<WithStyles<typeof pageStyles>>
{
  public render()
  {
...
export default withStyles( pageStyles )( ChangelogPage );
